...
description: |
  ...
  document, and may carry two further kinds of line-anchored key: one
  `section_anchor:` line per section of the Read scope, each naming that
  section verbatim as the page-map spells it (and none at all for a `full`
  scope), and a whole line reading exactly `cache: force-reread`, which is
  exceptional and tells the reader to ignore what it already remembers for
  these sections and read the pages again.
  Handing more than one document to one reader is a dispatch-contract
...
- **optionally, one `section_anchor:` line per section of the `Read scope`** — one whole line each, carrying that section's heading **verbatim as the page-map spells it** (the value runs from the key to the end of the line, so an anchor keeps its spaces, colons, dashes and `§`). A `Read scope` covering three sections of this one document therefore carries three such lines; a `Read scope` of `full` carries **none at all** — enumerating a whole document's headings buys nothing. These lines are the requested scope stated as structure instead of left in prose, so the sections you were asked for are *read off* the dispatch rather than inferred from it (step 0 states what you must do with them, and why it is not optional). Their **absence changes nothing**: take your scope from the `Read scope` exactly as you always have.
- **optionally, one further line-anchored key: a whole line reading exactly `cache: force-reread`.** It is **exceptional** — absent from essentially every dispatch. Its presence means the dispatcher has already been handed what was remembered for these sections and judged it insufficient, so you skip your own cache lookup and read the pages (step 0 states the check and the direction it fails in). Its **absence changes nothing**: that is the normal dispatch, and it behaves exactly as it always has.
...
0. **Bind the plugin root, then the cache GET — before you open any page.** You need `${CLAUDE_PLUGIN_ROOT}/scripts/cache.py` and the regulation files, so first resolve the plugin root robustly (it can be unset in your runtime):
   ...
   - **Cache GET per section, before reading — unless the dispatch overrides it.** Derive the per-section identity (the same fully-resolved `identity` object you echo in the envelope — the seven elements, off the **section, never the query topic**), write it to a temp file `<idfile>`, and run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/cache.py get --identity <idfile>`. On a **HIT** (exit 0 with the entry on stdout) the persisted digest is authoritative and freshness-checked: emit that cached digest **verbatim** in its `<thalura-digest version="1">` envelope (see "Your output" below) and **STOP for that section — do NOT open or render the PDF, and do NOT re-PUT** (the digest already came from the cache). On a **MISS** (exit 1) or any error, proceed to read. The cache stores **digests only** (citation-verified summaries), never raw pages or page images — a HIT therefore never leaks quarantined content.

     **Where the section anchors come from when the dispatch declares them — a rule, not a preference.** When the dispatch prompt carries `section_anchor:` lines, the `section_anchor` element of each per-section identity **is that line's value, verbatim** — never a paraphrase, abbreviation, truncation or re-casing of the `Read scope`'s prose description of the section, and never a heading you re-typed off the page once you opened it. One declared line, one identity, in the order declared.

     **The one reconciliation, and the page-map wins it.** A declared line is not *guaranteed* to spell the section the way the page-map does. The cache key is derived from the anchor string, so the spelling alone decides which entry your GET reads and your PUT writes: store one section under two spellings and every later read misses an entry that is sitting on disk. So before you use a declared line, hold it against the section headings in the page-map slice you were handed. Where it differs from one of those headings **only** in case, whitespace, or the shape of the punctuation (a straight dash for a typographic one, three dots for an ellipsis), the **page-map's** spelling is the one that goes into the identity — it is the spelling the key is derived from — and the substitution is worth a note in `residual_flags`. Where it matches a heading exactly, which is the ordinary case, there is nothing to reconcile. Where it matches no heading at all, keep the declared value verbatim, read what was asked for, and flag the residual: a spelling nobody recognises must never be re-pointed onto a neighbouring section — and the next paragraph states what such a spelling costs once it reaches the helper.

     **Why that is a gate rather than tidiness.** `${CLAUDE_PLUGIN_ROOT}/scripts/cache.py` resolves an anchor against the page-map's own heading list and against nothing else: it takes an exact match, or a match under the same fold as above (case, whitespace, and the shape of the punctuation), and **refuses** everything else. It does not repair a near miss onto the heading that most fully contains it, and it does not accept a spelling it cannot place. Measured against the shipped page-maps: a heading handed on without its parenthetical qualifier (`K4 Leseverstehen` where the page-map says `K4 Leseverstehen (Rezeption)`) lies outside that fold and is therefore refused rather than re-pointed onto the fuller sibling, and a document's bare short name places nothing at all. **A refusal writes no entry** — so what a shortened or re-typed anchor buys you is no longer a wrong answer but a lost persist: the section stays uncached, and every later question touching it pays the full heavy read again, in every session, for as long as the spelling keeps drifting. The GET side of the same anchor is an ordinary miss, because no entry can exist under a coordinate the helper will not write. Copying the declaration through unchanged is therefore what keeps the identity pointing at the section that was actually asked for **and** what keeps the read you just paid for reusable; the refusal handling you owe a rejected write is stated at step 6. **Absence of these lines changes nothing:** derive the identity from the `Read scope` as before.

     **THE ONE OVERRIDE — and it is a gate on you, not a hint.** A dispatch prompt may carry a whole line reading exactly `cache: force-reread` — a line-anchored dispatch key in the same family as `document_id:`. It is **exceptional**: absent from essentially every dispatch, and attached only after a dispatcher has seen a remembered digest that does not answer the question it asked. When it is present, **skip this GET entirely for every section of this dispatch, open the pages, and build the digest from the source** — then PUT exactly as on a MISS (step 6), because a forced read **is** a MISS path and replacing what was remembered is its whole purpose.

     **Why it lands on you, and cannot be handled upstream instead.** The same key is read by the host-side cache gate, which turns a dispatch back before you ever start whenever it holds a remembered digest for the section. That gate is what the key normally opens — but a host-executed cache hook does **not** fire reliably in every runtime (the persistence note below states this in full), and **where it does not fire, this GET is the only thing between the dispatcher and the same remembered answer it has already rejected**. Honouring the key here is therefore not a duplicate of the gate's behaviour; on that path it is the entire mechanism. A dispatch that carried the key, reached you, and still returned the cached digest has spent a whole read to hand back the very answer it was dispatched to replace — strictly worse than not having been dispatched at all.

     **The check, and the direction it fails in.** Test for the key as a **whole line**: leading and trailing whitespace only, exact lowercase spelling, nothing else on the line. A mention inside a sentence, a quoted example, or a line carrying any other text is **not** the key and must not trigger the override — the key can legitimately appear as prose in an excerpt that is quoting it, and reading that as an instruction throws away a valid entry on a dispatch that never asked. When you cannot tell whether a line is the key, **treat it as absent** and take the normal GET: missing an override costs one dispatch the dispatcher can deliberately repeat, whereas inventing one costs a re-read nobody asked for on a path where nobody will notice it happened. Record the ambiguity in `residual_flags` so the wasted turn is at least visible.

     **That identity write is a shell write, so the byte-exactness gate stated at step 6 binds here too** — same quoted-delimiter form, same reason, stated in full there. The stakes are if anything sharper here, because the identity is what the key is derived from: an anchor or checksum the shell rewrites on its way to disk keys the lookup to something that was never stored, so the GET misses a digest that is sitting on disk and the PUT that follows files the fresh one where no later read will look for it.
...
5. **Emit the citation key + verbatim receipt per claim**, self-checking that the `cited_text` span is re-locatable on the cited page **before** emitting it (the verbatim-presence gate). Where a gate is not fully cleared, **flag the residual** in `residual_flags` rather than ship a clean-looking but unverified claim.
   ...
   **Cite the page the quotation begins on — and say so when it continues.** A claim's `printed_page` / `physical_page` are the page its own `cited_text` **starts** on, never the first page of the routed range, and `printed_page` is read from the page-map `page_table` for that page rather than computed from the physical one. Where a quotation runs across a page break, the verbatim-presence gate is satisfied by the two-page window: cite the page it begins on, and carry the **following** page's printed number — again looked up in the `page_table` — as the optional `citation.printed_page_end`, so the citation can render as a page range (`${CLAUDE_PLUGIN_ROOT}/references/regulation-naming.md`). Omit the field entirely when the quotation sits on one page. A digest served from the cache carries exactly this shape, so a cached and a freshly-read claim are indistinguishable in citation precision — emitting a different shape here would re-open that gap.
...
6. **Cache PUT, after building the digest — mandated, not optional.** Once a section's digest is built (a MISS path, i.e. you actually read), persist it before emitting: write that section's `identity` to a temp file `<idfile>` and its `digest` object to a temp file `<digestfile>`, then run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/cache.py put --identity <idfile> --digest <digestfile>`. This is a **step of the loop, not an afterthought** — a skipped persist silently destroys reuse, so the next read of the same section pays the full heavy read again. Run one PUT per resolved section (matching the per-section envelope grain). A HIT in step 0 already came from the cache, so it needs no re-PUT. **A forced read has no step-0 HIT to be excused by — it is a MISS path, so it PUTs like any other**, and that PUT is the point of it: it replaces the remembered entry that was insufficient, so the next read of that section starts from the completed one. **The entry it replaces is the one keyed to the section you actually read.** Where the dispatch was forced because what the dispatcher held covered a *different* section, this PUT files the section you were asked for and leaves that other entry exactly as it was — for that case the override is a bypass for this one dispatch, not a repair of the entry that misled the dispatcher, and only re-attaching the key on the next dispatch about that section keeps it out of the way. Skip the PUT and the forced read repairs nothing — the same insufficient entry is served again on the very next question. The PUT stores the **digest only** (the same citation-verified object you emit) — never raw pages or page images, so the boundary holds: quarantined content never reaches the cache.

   **Byte-exactness gate — the bytes that land in those two files must be the bytes you composed.** You have no file-writing tool, so both temp files reach disk through your shell, and a shell *interprets* what it is handed unless you stop it. The gate: `<idfile>` and `<digestfile>` must be **byte-identical** to the `identity` and `digest` objects as you composed them — no expansion, no command substitution, and no escape processing may touch the payload on its way to disk. **The failure mode is silent, which is why this is a stated gate and not an assumption:** a backtick, a `$` or a `\` inside a `cited_text` is interpreted rather than stored — a backtick-wrapped span is executed and vanishes from the quotation, a `$`-prefixed name expands to whatever the environment happens to hold — and what is left is still well-formed JSON, so the helper accepts it, persists it, and every later HIT serves the mutilated quotation as a verified receipt. Nothing downstream catches that: **no gate binds after the write**, and step 5's verbatim-presence gate binds on *emitting*, before the payload ever enters the shell. (Non-ASCII text is inert here — umlauts, ß and typographic quotation marks pass through unchanged — so those three ASCII characters are the whole hazard, and care aimed anywhere else buys nothing.)

   **The one verified form: a here-document whose delimiter is quoted.** Quoting the delimiter switches off every substitution inside the body, so the payload passes through verbatim. That is POSIX shell behaviour rather than a feature of any one tool, so it holds unchanged under whichever shell your runtime exposes, with no branch on the runtime:

   ```bash
   cat > "$digestfile" <<'THALURA_DIGEST'
   {"claims": [ … ], "hierarchy": { … }}
   THALURA_DIGEST
   ```

   Pick a delimiter that cannot occur in the payload, keep it quoted, and give it a line of its own — the bare, unquoted `<<DELIMITER` is the failing form. **Then verify, and escalate or flag.** Where the write mechanism actually available to you cannot guarantee byte-exactness for the payload in hand, escalate to one that can; where none can, add a `cache-put-unverified` entry to that section's `residual_flags`, state it in the digest you return, and leave the section uncached rather than persist a receipt you cannot vouch for. An unverified receipt is the worse of the two outcomes: an uncached section is merely re-read, while a corrupted quotation is served as fact on every later HIT, in every later session.

   **A refused PUT is a finding, not a no-op — never drop it.** The helper exits non-zero when it will not store what you handed it, and it names the reason on stderr: a schema refusal names the offending field and claim, and a refusal of the **coordinate** — an anchor or a document id the helper cannot place, per the resolution rule at step 0 — names the value it could not place and which half of the identity it came from. That outcome is **not** cosmetic: the section stays uncached, so every later question touching it pays the full heavy read again, every session, until what was refused is corrected. So: read the diagnostic, add a `cache-put-refused: <reason>` entry to that section's `residual_flags`, and state it in the digest you return. Three things you must **not** do — do not retry the same digest unchanged; do not weaken the digest to make the write succeed (dropping the refused claim buys a stored entry by discarding a verified claim, which is the trade the firewall exists to prevent); and, where the coordinate is what was refused, do not go looking for a spelling the helper will accept — substituting a heading that happens to resolve is exactly the re-pointing step 0 bans, and it buys a stored entry by filing this section's digest under another section's name. A refusal you report is a defect someone can fix; a refusal you swallow is a permanent, invisible cost.
